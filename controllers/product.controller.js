...
export const getProductById = async (req, res) => {
  try {
    ...
    if (q !== "all" && Array.isArray(responseProduct.variantId) && responseProduct.variantId.length > 0) {
      responseProduct.variantId = [responseProduct.variantId[0]];
    }

    return res.status(200).json({ success: true, message: "Product fetched successfully", result: responseProduct });
  } catch (error) {
